...
Interfaz grafica del nodo, escrita con tkinter.
"""
...
def _crear_tarjeta(padre, clave, fila, columna):
    ...
    tk.Label(marco, text=sensores.etiqueta(clave), bg=TARJETA,
             fg=TENUE, font=("Segoe UI", 10, "bold"), anchor="w").pack(
...
def _actualizar_procesos(lecturas):
    ...
    if lectura is None:
        ...
        return
    _lista_procesos.insert(tk.END,
        f" {'PID':>7} {'PROCESO':<24} {'CPU':>7} {'RAM':>7}")
...
def _alternar_pausa():
    ...
    registro.registrar_evento(
        "INFO", "usuario",
        "Monitoreo en pausa" if _pausado else "Monitoreo reanudado")
...
def _refrescar():
    ...
    _reloj.config(text=time.strftime("%H:%M:%S"))
...
def iniciar():
    ...
    tk.Label(cabecera, text=config.NODO, bg=FONDO, fg=TEXTO,
